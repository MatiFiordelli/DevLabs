  ...
  const updatedState = todoEntriesList?.map((entry, index) => {
    if (index === i) {
      if (
        ...
        isItARepeatedEntry(localEntry?.updatedText as string, todoEntriesList)
      ) {
      ...
      return {
        ...entry,
        entryText: entry.isEditButtonActive
...
export const onSubmitFormTodoEntry = (
	...
	if(isValidLength(localEntry?.entryText)) {
		if(!isItARepeatedEntry(localEntry?.entryText as string, todoEntriesList)){
